...
// ─────────────────────────────────────────────────────────────────────────────
// Operações Protheus — SEÇÕES compartilhadas (C2).
// Blocos presentacionais extraídos da antiga Visão Geral (F4) para serem
// REUTILIZADOS pelas páginas decompostas (Ambientes / AppServers / Compilação /
// Patches / RPO) SEM duplicar lógica. Nada de novo comportamento: são os MESMOS
// cards/tabelas/monitores/console do F4, agora exportados. 100% via datasource
// (fixture no F4); ações continuam simuladas pelo runner (operations.tsx).
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  Badge, Button, Card, Select, Table, Tbody, Td, Th, Thead, Tr,
} from '@/components/ds'
import { getOperacoesDataSource } from '@/lib/operacoes/datasource'
import type {
  ChangeEntry, ConsoleLog, ConsoleSource, FolderStatus, ServiceRow, SystemInfo,
} from '@/lib/operacoes/types'
import {
  CHANGE_TYPE_META, CHART_PALETTE, CHART_TOOLTIP_STYLE, FOLDER_LEVEL_META,
...
/** Linha de 4 indicadores de saúde (reutilizada por AppServers e Visão Geral). */
export function HealthStatCards({ health }: { health: HealthSummary }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <StatCard label="Saúde do ambiente" value={health.label} color={health.color} icon={Activity} />
      <StatCard label="Serviços iniciados" value={String(health.running)} color="var(--success)" icon={PlayCircle} />
      <StatCard label="Serviços parados" value={String(health.stopped)} color={health.stopped ? 'var(--danger)' : 'var(--text)'} icon={StopCircle} />
      <StatCard label="Degradados" value={String(health.degraded)} color={health.degraded ? 'var(--warning)' : 'var(--text)'} icon={AlertTriangle} />
    </div>
  )
}

export function SectionHead({ icon: Icon, title, subtitle, children }: { icon: typeof Server; title: string; subtitle?: string; children?: React.ReactNode }) {
  return (
    ...
    <Card padding="none">
      <SectionHead icon={Server} title="AppServers" subtitle="Serviços do ambiente Protheus">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button size="sm" variant="secondary" icon={PlayCircle} disabled={!canControl} onClick={() => onServiceAll('start')}>Start All</Button>
          <Button size="sm" variant="secondary" icon={StopCircle} disabled={!canControl} onClick={() => onServiceAll('stop')}>Stop All</Button>
        </div>
      </SectionHead>
      <Table>
        <Thead>
...
export function InfoGrid({ info }: { info: SystemInfo }) {
...
/** Card "Informações do Ambiente" (INI) pronto. */
export function InfoCard({ info }: { info: SystemInfo }) {
  return (
    <Card>
...
export function FolderMonitor({ folder }: { folder: FolderStatus }) {
...
/** Card "Monitoramento · pasta System" pronto. */
export function FolderMonitorCard({ folder }: { folder: FolderStatus }) {
  return (
    <Card>
      <SectionHead icon={FolderCog} title="Monitoramento · pasta System" />
      <FolderMonitor folder={folder} />
    </Card>
  )
}

/** Botão de operação (grande, com descrição). */
export function OpButton({ icon: Icon, title, desc, disabled, danger, onClick }: { icon: typeof Server; title: string; desc: string; disabled?: boolean; danger?: boolean; onClick: () => void }) {
  return (
...
// ── Console viewer (log dos appservers) ───────────────────────────────────────
export function ConsoleViewer({ environmentId }: { environmentId: string | null }) {
...
// ── Histórico de mudanças (reuso do padrão de Mudanças) ────────────────────────
/** Tabela de histórico (compile/patch/promote/rollback) — mesma UX de Mudanças. */
export function ChangeHistoryTable({ rows }: { rows: ChangeEntry[] }) {
  const [openId, setOpenId] = useState<string | null>(null)
  if (rows.length === 0) {
    return <div className="px-5 py-8 text-center text-sm" style={{ color: 'var(--text-light)' }}>Nenhum registro para este ambiente ainda.</div>
  }
  return (
    <Table>
      <Thead>
