...
import { EXTRA_PERMISSION_OPTIONS } from '@/lib/permissions-catalog'
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <div className="flex items-center gap-1 bg-[var(--surface)] border border-[var(--border)] rounded-full px-1.5 py-1">
          {[
            { value: '',              label: 'Todos' },
            { value: 'consultor',      label: 'Consultor' },
            { value: 'coordenador',    label: 'Coordenador' },
        ...
        <div className="mb-3 flex items-center gap-2 flex-wrap px-3 py-2.5 rounded-lg bg-[var(--primary-soft)] border border-blue-500/20 text-xs">
          ...
          {bulkField === 'hourly_rate' && (
            <Input type="number" min="0" step="0.01" value={bulkValue}
              ...
              placeholder="0,00"
              className="w-28 bg-[var(--surface-hover)] border-[var(--border)] text-[var(--text)] h-7 text-xs" />
          )}

          <Button onClick={applyBulk} disabled={!bulkValue || bulkApplying}
            className="h-7 text-xs bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-[var(--primary-fg)] px-3">
            {bulkApplying ? 'Aplicando...' : 'Aplicar'}
          </Button>
          <button onClick={clearSelection} className="text-[var(--text-light)] hover:text-[var(--text)] ml-auto">
            <X size={13} />
          </button>
        </div>
      )}
            ...
            <tr className="border-b border-[var(--border)] bg-[var(--surface)]">
              ...
              <th className="px-3 py-2.5 w-8">
                <input type="checkbox"
                  ...
                  className="rounded border-[var(--border-strong)] bg-[var(--surface-hover)] accent-[var(--primary)] cursor-pointer" />
              </th>
      ...
      {modal.open && (
        <ModalOverlay onClose={() => setModal({ open: false })}>
          <div className="p-5 max-h-[90vh] overflow-y-auto space-y-4">
            <h3 className="text-sm font-semibold text-[var(--text)]">{modal.item ? 'Editar Usuário' : 'Novo Usuário'}</h3>
            ...
            {form.profiles.length > 0 && (
              <>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        <div className="flex rounded-lg border border-[var(--border)] overflow-hidden text-xs">
                          {(['hourly', 'monthly'] as const).map(t => (
                            <button key={t} type="button" onClick={() => setForm(f => ({ ...f, rate_type: t }))}
                              className={`px-3 py-1.5 font-medium transition-colors ${form.rate_type === t ? 'bg-[var(--primary)] text-[var(--primary-fg)]' : 'bg-[var(--surface-hover)] text-[var(--text-muted)] hover:text-[var(--text)]'}`}>
                      ...
                      <CurrencyInput value={form.hourly_rate}
                        ...
                        placeholder="0.000,00" className="flex-1 bg-[var(--surface-hover)] border-[var(--border)] text-[var(--text)] h-8 text-xs" />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Input type="number" min="1" max="24" step="0.5" value={form.daily_hours}
                        ...
                        placeholder="8" className="w-24 bg-[var(--surface-hover)] border-[var(--border)] text-[var(--text)] h-8 text-xs" />
                ...
                {(isConsultor || isParceiroAdm) && (
                  <div>
                    <Label className="text-xs text-[var(--text-muted)] mb-1 block">Data de início do banco de horas</Label>
                    <Input
                      type="date"
                      value={form.bank_hours_start_date}
                      onChange={e => setForm(f => ({ ...f, bank_hours_start_date: e.target.value }))}
                      className="w-48 bg-[var(--surface-hover)] border-[var(--border)] text-[var(--text)] h-8 text-xs"
                    />
                ...
                {form.profiles.length > 0 && !form.profiles.includes('administrator') && (() => {
                  ...
                  return (
                    <div>
                      <Label className="text-xs text-[var(--text-muted)] mb-1 block">Permissões adicionais</Label>
                                ...
                                return (
                                  <label key={p.value} className={`flex items-center gap-2 ${isBase ? 'cursor-default' : 'cursor-pointer group'}`}>
                                    <input
                                      ...
                                      className="rounded border-[var(--border-strong)] bg-[var(--surface-hover)] accent-[var(--primary)] cursor-pointer disabled:cursor-default disabled:opacity-50"
                                    />
                                    <span className={`text-xs transition-colors ${isBase ? 'text-[var(--text-light)]' : 'text-[var(--text-muted)] group-hover:text-[var(--text)]'}`}>
                                      {p.label}
                                      {isBase && <span className="ml-1.5 text-[10px] text-[var(--text-muted)]">(padrão do perfil)</span>}
                                    </span>
                                  </label>
                                )
                              })}
            ...
            <div className="flex gap-2 pt-1 justify-end">
              ...
              <Button onClick={save} disabled={saving || !canSave} className="h-8 text-xs bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-[var(--primary-fg)]">
          ...
          <div className="p-5">
            {!resetModal.confirmed ? (
              <>
                ...
                <p className="text-xs text-[var(--text-muted)] mb-1">Uma nova senha temporária será gerada para:</p>
                ...
                <p className="text-xs text-[var(--text-light)] mb-5 p-3 rounded-lg bg-[var(--surface-hover)] border border-[var(--border)]">
                  A senha será exibida na tela e um <span className="text-[var(--text)] font-medium">e-mail será enviado automaticamente</span> ao usuário.
                </p>
          ...
          <ModalOverlay onClose={() => setViewUser(null)}>
            <div className="p-5">
              ...
              <div className="space-y-2.5">
                {rows.map(row => (
                  <div key={String(row.label)} className="flex items-center justify-between py-2 border-b border-[var(--border)] last:border-0">
              ...
              <div className="flex gap-2 mt-5 justify-end">
                ...
                <button onClick={() => setViewUser(null)}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium bg-[var(--surface-hover)] hover:bg-[var(--border-strong)] text-[var(--text)]">
                  Fechar
                </button>
              </div>
            </div>
