...
export default function RelatorioApontamentosPage() {
  ...
  // Título VISÍVEL limpo — período só nos campos Competência/Digitação do cabeçalho
  // (não embutir range no título; a digitação pode cruzar meses e confunde a competência).
  const reportTitle = useMemo(
    () => `Relatório de ${isFechamento ? 'Fechamento' : 'Apontamentos'}`,
    [isFechamento]
  )

  // Base do NOME DO ARQUIVO (PDF/Excel) — aqui SIM leva o período: digitação se houver,
  ...
  function handlePrint() {
    ...
    // Nome sugerido do PDF: base datada + cliente (ex.: "...25 de Junho a 02 de Julho de 2026 — VEDAMOTORS")
    document.title = customerName ? `${reportFileBase} — ${customerName}` : reportFileBase
    const cleanup = () => {
      clone.remove()
                              ...
                              <Fragment key={`${gi}-${i}`}>
                                <tr style={{ background: bg, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' } as React.CSSProperties}>
                                  ...
                                  <td className="px-3 pt-2 pb-1 text-xs text-gray-500 text-center">
                                    {t.ticket
                                      ? <a href={`/help-desk/tickets/${(t as any).helpdesk_ticket_id ?? t.ticket}`} target="_blank" rel="noopener noreferrer" className="text-[#5b21b6] hover:text-[#5b21b6]">#{t.ticket}</a>
                                      : '—'}
                                  </td>
                      ...
                      {ticketSummary.map((tk, i) => (
                        ...
                        >
                          <td className="px-3 py-2 text-xs whitespace-nowrap">
                            <a href={`/help-desk/tickets/${(tk as any).helpdesk_ticket_id ?? tk.ticket}`} target="_blank" rel="noopener noreferrer" className="text-[#5b21b6] hover:text-[#5b21b6]">
                              #{tk.ticket}
                            </a>
                          </td>
              ...
              {sortedItems.map((t, i) => (
                <Tr key={i}>
                  <Td className="whitespace-nowrap">
                    <span
                      style={isLateInclusion(t) ? { color: '#b45309', fontWeight: 600 } : undefined}
