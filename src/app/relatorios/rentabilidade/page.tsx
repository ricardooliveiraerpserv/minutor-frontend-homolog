  ...
  useEffect(() => {
    ...
    Promise.all(monthsToFetch.map(ym =>
      api.get<{ data: { rows: Row[]; por_dia?: DiaRow[]; fixos_zerados?: FixoZerado[] } }>(`/relatorios/rentabilidade/${ym}${clamp(ym)}`)
        .then(r => ({ ym, rows: (r?.data?.rows ?? []).map(row => ({ ...row, is_investimento: !!row.is_investimento || isErpsRow(row.cliente) })), dias: r?.data?.por_dia ?? [], fixos: r?.data?.fixos_zerados ?? [] })).catch(() => ({ ym, rows: [] as Row[], dias: [] as DiaRow[], fixos: [] as FixoZerado[] }))
    )).then(perMonth => {
      setMonthly(perMonth)
