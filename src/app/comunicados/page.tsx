...
const TIPO_L: Record<string, string> = { aviso: 'Aviso', formal: 'Comunicação formal', campanha: 'Campanha' }
...
export default function ComunicadosPage() {
  ...
  // Re-busca o corpo renderizado no tema atual (cores legíveis no claro/escuro).
  useEffect(() => {
    if (!resolvedTheme) return
    const theme = resolvedTheme === 'dark' ? 'dark' : 'light'
    api.get<{ data: Comunicado[] }>(`/communications/mine?theme=${theme}`)
      .then(r => {
        const d = r.data ?? []; setRows(d); setOpen(o => o ?? (d.length ? d[0].id : null))
        if (d.some(c => !c.read)) api.post('/communications/mark-read', {}).catch(() => {})
      })
      .catch(() => {})
  ...
  return (
    <AppLayout title="Comunicados">
      <div className="space-y-4 max-w-3xl">
        <div className="flex items-center gap-2.5">
          <Megaphone size={22} style={{ color: 'var(--primary)' }} />
          <h1 className="text-lg font-semibold" style={{ color: 'var(--text)' }}>Comunicados</h1>
          <span className="text-xs" style={{ color: 'var(--text-light)' }}>· Avisos e comunicações da ERPSERV</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="ds-card p-8 text-center">
            ...
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Nenhum comunicado no momento.</p>
          </div>
        ) : (
