      ...
      {loading ? <p style={{ color: 'var(--text-light)' }}>Carregando…</p>
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            <select value={f.crm_status} onChange={e => setF(s => ({ ...s, crm_status: e.target.value }))} className="px-3 py-2 rounded-lg text-sm outline-none" style={inputStyle}><option value="">Status CRM</option>{['lead','prospect','cliente','em_renovacao','inativo'].map(s => <option key={s} value={s}>{s}</option>)}</select>
