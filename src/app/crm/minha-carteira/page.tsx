      ...
      {loading ? <p style={{ color: 'var(--text-light)' }}>Carregando…</p>
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            {d.pode_ver_todos && <select value={f.executivo_id} onChange={e => setF(s => ({ ...s, executivo_id: e.target.value }))} className="px-3 py-2 rounded-lg text-sm outline-none" style={inputStyle}><option value="">Meu / todos</option>{d.executivos.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select>}
            ...
            <select value={f.crm_status} onChange={e => setF(s => ({ ...s, crm_status: e.target.value }))} className="px-3 py-2 rounded-lg text-sm outline-none" style={inputStyle}><option value="">Status CRM</option>{['lead','prospect','cliente','contrato_ativo','em_renovacao'].map(s => <option key={s} value={s}>{s}</option>)}</select>
