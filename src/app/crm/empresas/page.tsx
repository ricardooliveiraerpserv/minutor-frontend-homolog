...
const STATUS: { v: string; l: string; bg: string; fg: string }[] = [
  ...
  { v: 'contrato_ativo', l: 'Contrato Ativo', bg: 'rgba(16,185,129,0.18)', fg: '#10b981' },
  ...
  return (
    <AppLayout title="Empresas (CRM)">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-4">
        {(['lead', 'prospect', 'cliente', 'contrato_ativo', 'em_renovacao'] as const).map(v => {
          const si = statusInfo(v); const n = list.filter(c => c.crm_status === v).length
          return (
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs mb-1" style={{ color: 'var(--text-muted)' }}>
                  CNPJ / CPF {['cliente', 'contrato_ativo', 'em_renovacao'].includes(crmStatus) && <span style={{ color: 'var(--danger-border)' }}>* obrigatório p/ cliente</span>}
                </label>
