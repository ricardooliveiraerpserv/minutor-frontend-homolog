...
import { ArrowLeft, Plus, Check, Clock, AlertTriangle, Trophy, FileDown, Trash2, Building2, User, Package, ListChecks, History as HistoryIcon, FileText, Paperclip } from 'lucide-react'
...
export default function OportunidadeDetalhePage() {
  ...
  const [tab, setTab] = useState<'atividades' | 'historico' | 'propostas' | 'anexos'>('atividades')
  // Aba inicial via ?tab= (client-only, sem useSearchParams → dispensa Suspense no build).
  useEffect(() => {
    const t = new URLSearchParams(window.location.search).get('tab')
    if (t === 'historico' || t === 'propostas' || t === 'anexos') setTab(t)
  }, [])
  ...
  if (loading) return <AppLayout title="Oportunidade"><p style={{ color: 'var(--text-light)' }}>Carregando…</p></AppLayout>
  if (!o) return <AppLayout title="Oportunidade"><p style={{ color: 'var(--text-light)' }}>Oportunidade não encontrada.</p></AppLayout>
    ...
    <AppLayout title="Oportunidade">
      ...
      <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
        <div className="min-w-0">
          <button onClick={() => router.back()} className="flex items-center gap-1 text-xs mb-1" style={{ color: 'var(--text-muted)' }}><ArrowLeft size={13} /> Voltar</button>
        ...
        {o.contract_id
          ? <span className="text-xs inline-flex items-center gap-1 px-3 py-2 rounded-lg" style={{ background: 'rgba(34,197,94,.12)', color: '#16a34a' }}><Trophy size={14} /> Convertida — contrato #{o.contract_id}</span>
      ...
      <div className="rounded-xl p-3 mb-4 flex items-center gap-2 flex-wrap" style={{ background: o.proxima_acao_at && new Date(o.proxima_acao_at) < new Date() ? 'var(--danger-bg)' : 'var(--surface-sunken)' }}>
        <Clock size={16} style={{ color: o.proxima_acao_at && new Date(o.proxima_acao_at) < new Date() ? 'var(--danger-border)' : 'var(--primary)' }} />
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: 'minmax(0,1fr) minmax(300px,360px)' }}>
                  ...
                  <div className="space-y-1">
                    {feitas.slice(0, 20).map(t => (
                      <div key={t.id} className="flex items-center gap-2.5 rounded-lg px-3 py-2" style={{ background: 'var(--surface-sunken)' }}>
                        <button onClick={() => toggleTask(t)} title="Reabrir" className="w-5 h-5 rounded-full shrink-0 flex items-center justify-center" style={{ background: 'var(--success-border, #16a34a)' }}><Check size={12} style={{ color: '#fff' }} /></button>
        ...
        {/* ───────── LATERAL: resumo/previsibilidade/produtos/contato ───────── */}
        <div className="space-y-4 self-start">
          ...
          {prods.length > 0 && (
            <div className="rounded-xl p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              ...
              <p className="text-[10px] mt-1.5" style={{ color: 'var(--text-light)' }}>Editar preço/custo/margem: no Kanban, abra o card &gt; Resumo &gt; Produtos.</p>
            </div>
          )}

          {/* Contato / empresa / responsável */}
          <div className="rounded-xl p-4 space-y-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
