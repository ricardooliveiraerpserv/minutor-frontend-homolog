        ...
        {extraValor > 0 ? (
          <>
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-semibold tabular-nums" style={{ color: 'var(--text)' }}><Money>{formatBRL(closing.total_servico - extraValor)}</Money></span>
            </div>
            <div className="flex items-start justify-between text-sm">
              <div className="min-w-0">
                <span style={{ color: 'var(--text-muted)' }}>+ Hora extra</span>
                {extraHoras > 0 && valorHora > 0 && (
                  <p className="text-[11px] mt-0.5" style={{ color: 'var(--text-light)' }}>{fmtHours(extraHoras)} × <Money>{formatBRL(valorHora)}</Money>/h</p>
                )}
              </div>
              <span className="font-medium tabular-nums shrink-0 ml-3" style={{ color: 'var(--success)' }}>+ <Money>{formatBRL(extraValor)}</Money></span>
            </div>
          </>
        ) : (
          <div className="flex items-center justify-between text-sm">
...
function HBPaymentSection({ data, fixedSalary, expTotal, expPaid, showExtras = true, recebimento = null }: { data: HourBankMonth; fixedSalary: number; expTotal: number; expPaid: number; showExtras?: boolean; recebimento?: number | null }) {
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-6 py-5 grid grid-cols-[auto_1fr_auto] gap-x-8 items-center">
      ...
      <div className="flex flex-col gap-1 border-l border-[var(--border)] pl-8">
        <div className="text-[42px] font-extrabold leading-none tracking-tight text-[var(--primary)]">
          <Money>{fixedSalary > 0 ? formatBRL(displayTotal) : '—'}</Money>
        </div>
        <span className="text-[11px] text-[var(--text-muted)] mt-1">
        ...
        {fixedSalary > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-sunken)] px-3 py-2">
              ...
              <p className="text-sm font-bold text-[var(--text)]"><Money>{formatBRL(valorHoraExt)}</Money><span className="text-[10px] font-normal text-[var(--text-muted)]">/h</span></p>
            </div>
            {hasExtra && (
              <div className="rounded-lg border border-green-500/20 bg-[var(--success-bg)] px-3 py-2">
                ...
                <p className="text-sm font-bold text-[var(--success)]"><Money>{formatBRL(totalExtra)}</Money></p>
                <p className="text-[9px] text-[var(--text-muted)]">{fmtHours(extraHours)} × <Money>{formatBRL(valorHoraExt)}</Money>/h</p>
              </div>
            )}
          </div>
        )}
