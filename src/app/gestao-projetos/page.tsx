        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              />
              {/* Help Desk: vincular CHAMADOS a este projeto (não-sustentação). Ligado, as interações
                  com tempo dos chamados viram apontamento neste projeto. Sustentação já nasce ligada. */}
              {(() => {
                const stName = (optServiceTypes.find(s => s.id === Number(form.service_type_id))?.name ?? '').toLowerCase()
                if (stName.includes('sustenta')) return null
                return (
                  <Toggle2
...
function ProjectEditByIdModal({ projectId, onClose, onSaved }: { projectId: number; onClose: () => void; onSaved: () => void }) {
  ...
  if (loading) return (
    <div className="p-4 md:p-6 space-y-2">
      <div className="h-9 rounded animate-pulse" style={{ background: 'var(--surface-hover)' }} />
      {[...Array(8)].map((_, i) => (
        <div key={i} className="h-12 rounded animate-pulse" style={{ background: 'var(--surface-hover)' }} />
      ))}
    </div>
  )
  if (!p) return null
