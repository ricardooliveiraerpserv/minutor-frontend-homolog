                    ...
                    <div key={c.id} className={`flex gap-2.5 ${right ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`flex flex-col min-w-0 ${editing ? 'w-full' : 'max-w-[85%]'} ${right ? 'items-end' : 'items-start'}`}>
                        ...
                        {editing ? (
                          ...
                          <div className="hd-msg-body text-sm text-left rounded-2xl px-3.5 py-2.5 w-full max-w-full overflow-hidden" style={{ background: '#ffffff', color: '#1f2937', border: `1px solid ${isInternal ? 'var(--warning-border)' : right ? 'var(--primary)' : '#e5e7eb'}`, borderTopRightRadius: right ? 4 : 16, borderTopLeftRadius: right ? 16 : 4 }}>
                            {isHtmlBody(c.body)
