                ...
                <div className="p-4 space-y-4">
                  ...
                  {comments.slice().reverse().map(c => {
                    ...
                    <div key={c.id} className={`flex gap-2.5 ${right && !isHtmlBody(c.body) ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`flex flex-col ${editing ? 'w-full min-w-0' : isHtmlBody(c.body) ? 'w-max max-w-[calc(100vw_-_48px)] shrink-0' : 'min-w-0 max-w-full'} ${right && !isHtmlBody(c.body) ? 'items-end' : 'items-start'}`}>
                  ...
                  {t.description && (() => {
                    ...
                    <div className="flex gap-2.5">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 text-white" style={{ background: avatarColor(autor) }}>{iniciais(autor)}</div>
                      <div className={`flex flex-col items-start ${editDesc ? 'w-full min-w-0' : isHtmlBody(t.description) ? 'w-max max-w-[calc(100vw_-_48px)] shrink-0' : 'min-w-0 max-w-full'}`}>
                        <div className="flex items-center gap-2 mb-1 text-[11px]">
                          <span className="font-semibold" style={{ color: 'var(--text)' }}>{autor}</span>
                        ...
                        {editDesc ? (
                          ...
                          <div className={`text-sm text-left rounded-2xl px-3.5 py-2.5 relative z-[1] ${isHtmlBody(t.description) ? 'w-max max-w-[calc(100vw_-_48px)]' : 'hd-msg-body w-fit max-w-full'}`} style={{ background: '#ffffff', color: '#1f2937', border: '1px solid #e5e7eb', borderTopLeftRadius: 4 }}>
