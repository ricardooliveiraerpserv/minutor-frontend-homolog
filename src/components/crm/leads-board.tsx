...
export function LeadsBoard() {
  ...
  return (
    <div>
      ...
      {rankingOpen && (
        <div className="rounded-xl mb-3 p-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {ranking.map(r => (
                <tr key={r.responsavel} style={{ borderTop: '1px solid var(--border)' }}>
                  <td className="py-1.5" style={{ color: r.responsavel === 'Sem responsável' ? 'var(--danger-border)' : 'var(--text)' }}>{r.responsavel}</td>
                  <td className="text-right tabular-nums" style={{ color: 'var(--text)' }}>{r.ativos}</td>
                  <td className="text-right tabular-nums" style={{ color: r.sem_proxima ? 'var(--warning-border)' : 'var(--text-light)' }}>{r.sem_proxima}</td>
                  <td className="text-right tabular-nums" style={{ color: r.atrasados ? 'var(--danger-border)' : 'var(--text-light)' }}>{r.atrasados}</td>
                  <td className="text-right tabular-nums" style={{ color: 'var(--text-muted)' }}>{r.em_risco}</td>
                  <td className="text-right tabular-nums" style={{ color: 'var(--text-muted)' }}>{fmtBRL(r.valor)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Minha Agenda Comercial (CrmTasks) */}
      <div className="rounded-xl mb-3" style={{ background: 'var(--surface-sunken)', border: '1px solid var(--border)' }}>
        <button onClick={() => setWorklistOpen(o => !o)} className="w-full flex items-center justify-between px-3 py-2">
          <span className="text-xs font-bold uppercase tracking-wider flex items-center gap-2 flex-wrap" style={{ color: 'var(--text-light)' }}>
            <Clock size={13} /> Minha agenda comercial
            <span className="px-1.5 rounded-full" style={{ background: 'var(--danger-bg)', color: 'var(--danger-border)' }}>{atrasadas.length} atrasadas</span>
            <span className="px-1.5 rounded-full" style={{ background: 'var(--warning-bg)', color: 'var(--warning-border)' }}>{hoje.length} hoje</span>
            <span className="px-1.5 rounded-full" style={{ background: 'var(--primary-soft)', color: 'var(--primary)' }}>{prox7.length} em 7 dias</span>
          </span>
          <span style={{ color: 'var(--text-light)' }}>{worklistOpen ? '▾' : '▸'}</span>
        </button>
        {worklistOpen && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 px-3 pb-3">
            {([['Atrasadas', atrasadas, 'var(--danger-border)'], ['Hoje', hoje, 'var(--warning-border)'], ['Próximos 7 dias', prox7, 'var(--primary)']] as const).map(([titulo, lista, cor]) => (
              <div key={titulo}>
                ...
                <div className="space-y-1">
                  ...
                  {lista.length === 0 && <p className="text-[10px]" style={{ color: 'var(--text-light)' }}>—</p>}
                </div>
              </div>
            ))}
          </div>
