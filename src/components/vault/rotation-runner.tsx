...
export function RotationRunner({ open, onClose, vaultId, vaultName, keyVersion, oldVaultKey, onDone }: RotationRunnerProps) {
  ...
  const run = async () => {
    ...
    try {
      ...
      toast.error(err instanceof ApiError ? err.message : err instanceof Error ? err.message : 'Falha na rotação.')
    } finally {
      setBusy(false)
      setProgress(null)
      setTotp('')
    ...
    <Modal open={open} onClose={() => { if (!busy) onClose() }} title={`Rotacionar chave — ${vaultName}`}>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button variant="primary" icon={RefreshCw} loading={busy} disabled={totp.length < 6} onClick={run}>
            Rotacionar agora
          </Button>
        </div>
      </div>
