...
export function AppLayout({ children, title, actions }: AppLayoutProps) {
  ...
  const [helpOpen, setHelpOpen] = useState(false)   // modal "Abrir chamado" da faixa do cliente
    ...
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: 'var(--bg)' }}>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          {/* Faixa "Precisa de ajuda?" — em todas as telas do cliente (menos o próprio portal).
              Abre o modal de chamado NA MESMA TELA (não navega). */}
          {isCliente && !pathname.startsWith('/help-desk') && (
            <div className="shrink-0 w-full flex items-center gap-2.5 px-4 md:px-6 py-2.5 border-b"
              style={{ borderColor: 'var(--border)', background: 'var(--surface-sunken)' }}>
              <LifeBuoy size={16} style={{ color: 'var(--primary)' }} className="shrink-0" />
              ...
              <button onClick={() => setHelpOpen(true)} className="ds-btn-primary text-sm font-medium inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg shrink-0">
