 ...
 * Só horas + operacional — SEM custo (R$) e SEM CPI/CV (o sistema não trabalha com custo).
 * Semáforo do spec: ≥1 verde, 0,9–1 amarelo, <0,9 vermelho.
 */
...
type Evm = { has_baseline: boolean; using_live_plan?: boolean; message?: string; baseline?: Baseline; as_of?: string; metrics?: Metrics; curve?: CurvePoint[] }
...
export function CronogramaEvmPanel({ projectId, canEdit }: { projectId: number; canEdit: boolean }) {
  ...
  const unfreeze = async () => {
    const ok = await confirm({
      ...
      message: 'Remover a linha de base? Os indicadores de EVM (SPI/curva-S) ficam indisponíveis até você congelar de novo. Os dados de progresso e apontamentos não são afetados.',
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[13px] mt-1" style={{ color: 'var(--text-muted)' }}>
            ...
            planejadas de cada etapa/atividade — depois disso o SPI passa a ter sentido, mesmo que o cronograma seja replanejado.
          </p>
          {canEdit ? (
...
function EvmSCurve({ curve }: { curve: (CurvePoint & { label: string })[] }) {
  const yFmt = (v: number) => `${Math.round(v)}h`
  const vFmt = (v: number) => fmtH(v)
