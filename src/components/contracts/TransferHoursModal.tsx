...
type TransferTarget = { id: number; label: string }
type TransferInfo = {
  source: { project_id: number; available_hours: number; rate: number }
 ...
 *
 * `project` precisa de: id, code (opcional), customer?.name (opcional, só p/ o cabeçalho).
 */
export function TransferHoursModal({ project, onClose, onDone }: {
  project: any | null
  onClose: () => void
  onDone: () => void
}) {
  ...
  useEffect(() => {
    if (!project) { setInfo(null); setToId(''); setHours(''); setDesc(''); return }
    setLoading(true)
    api.get<TransferInfo>(`/projects/${project.id}/transfer-info`)
      .then(setInfo)
  ...
  const apply = async () => {
    ...
    setSaving(true)
    try {
      const r = await api.post<any>(`/projects/${project.id}/transfer-hours`, {
  ...
  const srcLabel = `${project.customer?.name ?? ''}${project.code ? (project.customer?.name ? ' · ' : '') + project.code : ''}` || `Projeto #${project.id}`
      ...
      <div className="relative rounded-2xl p-6 w-full max-w-md mx-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="text-xs" style={{ color: 'var(--text-muted)' }}>Projeto destino (mesmo cliente)</label>
              <select value={toId} onChange={e => setToId(e.target.value)}
                className="mt-1 w-full rounded-xl px-3 py-2 text-sm"
                style={{ background: 'var(--bg)', border: '1px solid var(--border)', color: 'var(--text)' }}>
                <option value="">Selecionar projeto destino…</option>
                {(info?.targets ?? []).map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
              {info && info.targets.length === 0 && (
                <p className="text-[11px] mt-1" style={{ color: 'var(--warning)' }}>Nenhum outro projeto deste cliente.</p>
              )}
            </div>
