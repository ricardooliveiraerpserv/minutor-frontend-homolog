...
function RowActions({ id, onView, onDeleted, viewOnly, onExtraPct, onRelease, onReverseApproval, onReverseRelease, onReverseRejection, onShowLogs, onShowConflict, canEdit = true, canDelete = true, reasonEdit, reasonDelete }: {
  ...
  const items: RowMenuItem[] = viewOnly
    ...
    : [
        ...
        ...(onReverseApproval ? [{ label: 'Estornar aprovação', icon: <RotateCcw size={12} />, onClick: onReverseApproval }] : []),
          ...
          {items.map((item, i) => (
            <button key={i} disabled={item.disabled} title={item.title}
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 text-xs transition-colors text-left ${
                item.disabled ? 'text-[var(--text-muted)] cursor-not-allowed' : item.danger ? 'text-[var(--danger)] hover:bg-[var(--danger-bg)]' : 'text-[var(--text)] hover:bg-[var(--surface-hover)]'
              }`}>
...
function TimesheetsPageContent({ scope, embedded, triagemPadrao, leadOptions, extDate, lockedProjectId }: { scope?: 'sustentacao' | 'investimento'; embedded?: boolean; triagemPadrao?: boolean; leadOptions?: { id: number; name: string }[]; extDate?: PortalDate; lockedProjectId?: number } = {}) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (requester)     p.set('requester', requester)
      if (ticketService) p.set('ticket_service', ticketService)
      const res = await fetch(`/api/v1/timesheets/export?${p}`, { credentials: 'same-origin' })
