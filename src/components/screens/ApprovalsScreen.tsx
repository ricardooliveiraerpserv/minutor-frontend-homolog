...
export function ApprovalsScreen({ scope, embedded, leadOptions, extDate }: ApprovalsScreenProps = {}) {
  ...
  const filterParams = useMemo(() => {
    ...
    // o filtro persistido, que pode ser sobrescrito na hidratação (key muda com user.id) e
    // fazer a lista ignorar o mês. Isso também alinha "Ver como" à sessão real (mesma janela).
    ...
    return p.toString()
  }, [dateFrom, dateTo, userId, coordinatorId, executiveId, projectId, customerId, categoriaServico, isCoordenador, coordScope, user?.id, scope, extDate?.mode, extDate?.month, extDate?.year, extDate?.from, extDate?.to])
