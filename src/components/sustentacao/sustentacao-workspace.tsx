...
export function SustentacaoWorkspace({ show }: { show: 'central' | 'indicadores' }) {
  ...
  const [tab, setTab]         = useState(() => _searchParams.get('tab') || (show === 'indicadores' ? 'resumo' : ''))   // aba inicial via ?tab= (deep-link do menu)
  ...
  const [routineTab, setRoutineTab] = useState<RoutineTabId | null>(show === 'central' ? 'timesheets' : null)
  ...
  return (
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ background: 'var(--bg)' }}>
      ...
      {/* ── Central de Lançamentos — só no Portal (show='central') ── */}
      {show === 'central' && (
