...
/** Gestão dos centros de custo de um cliente: lista, criar/editar/excluir, importar Excel + modelo. */
export function CostCentersModal({ customerId, customerName, canEdit, onClose }: {
  ...
  const load = () => {
    setLoading(true)
    api.get<{ data: CostCenter[] }>(`/customers/${customerId}/cost-centers`)
  ...
  const salvar = async () => {
    ...
    setSaving(true)
    try {
      if (editing) await api.put(`/cost-centers/${editing.id}`, { code: c, description: d, active: editing.active })
      else await api.post(`/customers/${customerId}/cost-centers`, { code: c, description: d })
  ...
  const excluir = async (cc: CostCenter) => {
    ...
    try { await api.delete(`/cost-centers/${cc.id}`); toast.success('Centro de custo excluído'); if (editing?.id === cc.id) resetForm(); load() }
  ...
  const importar = async (file: File) => {
    ...
    try {
      ...
      const r = await api.post<{ message: string }>(`/customers/${customerId}/cost-centers/import`, fd)
  ...
  const baixarModelo = async () => {
    try {
      const res = await fetch('/api/v1/cost-centers/template', { credentials: 'same-origin' })
