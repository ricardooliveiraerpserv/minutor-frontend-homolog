...
// ─────────────────────────────────────────────────────────────────────────────
// Configuração COMPLETA de um ambiente (embutível) — Credenciais, Banco, AppServer,
// VPN, Certificados, Links, Documentação + Acesso Rápido, com os modais de cadastro.
// Mesmo conteúdo do detalhe de ambiente do Cofre, porém SEM AppLayout, para reuso no
// Prosight (Configuração → Connector). O Cofre não é alterado; aqui só consumimos os
// mesmos componentes/endpoints. Requer o vault destravado (zero-knowledge).
// ─────────────────────────────────────────────────────────────────────────────
...
export function EnvironmentConfigPanel({ envId }: { envId: number | string }) {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2">
          {perms.manage && <Button size="sm" icon={Pencil} onClick={() => setEditOpen(true)}>Editar</Button>}
          {perms.admin && <Button size="sm" icon={ShieldCheck} onClick={() => setShowPerms(true)}>Permissões</Button>}
        </div>
      </div>
